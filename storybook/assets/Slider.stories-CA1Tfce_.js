import{j as a}from"./jsx-runtime-D_zvdyIk.js";import{r as c}from"./iframe-BWNZ21XG.js";import{g as H,R as W}from"./responsive-_N8FZq_1.js";import{c as x}from"./cn-CNMN3A1O.js";import{w as K}from"./devWarn-C2PD3-_Q.js";import"./preload-helper-D9Z9MdNV.js";const pe={primary:"bg-gradient-to-r from-secondary to-primary shadow-primary",secondary:"bg-gradient-to-r from-primary to-secondary shadow-secondary",accent:"bg-gradient-to-r from-accent to-primary shadow-lg-accent"},fe={primary:"bg-primary shadow-primary",secondary:"bg-secondary shadow-secondary",accent:"bg-accent shadow-lg-accent"};function X(n,s){if(typeof n.setPointerCapture=="function")try{n.setPointerCapture(s)}catch{}}const u=({min:n=0,max:s=100,step:d=1,value:v,defaultValue:G,onValueChange:P,variant:z="accent",size:q="md",disabled:m=!1,label:p,showValue:$=!1,formatValue:y=D=>String(D),ariaLabel:L,ariaLabelMin:Z,ariaLabelMax:J,className:Q="",id:Y})=>{const D=c.useId(),E=Y||D,F=`${E}-label`;s<=n&&K(`slider-invalid-range-${n}-${s}`,`Slider: max (${s}) must be greater than min (${n}) — values will not update correctly.`),d<=0&&K(`slider-invalid-step-${d}`,`Slider: step (${d}) must be greater than 0.`);const[ee,ae]=c.useState(()=>v??G??n),I=v!==void 0,g=I?v:ee,i=Array.isArray(g),o=c.useMemo(()=>i?g:[g,g],[i,g]),U=c.useRef(null),[V,h]=c.useState(null),O=c.useRef(o);O.current=o;const _=c.useCallback(e=>{const r=d>0?d:1,t=Math.round((e-n)/r)*r+n,l=Math.min(s,Math.max(n,t));return Math.round(l*1e10)/1e10},[n,s,d]),f=c.useCallback((e,r)=>{const t=_(r),l=[...O.current];i?e===0?l[0]=Math.min(t,l[1]):l[1]=Math.max(t,l[0]):l[0]=t;const b=i?[l[0],l[1]]:l[0];I||ae(b),P?.(b)},[i,I,P,_]),M=c.useCallback(e=>{const r=U.current?.getBoundingClientRect();if(!r||r.width===0)return n;const t=(e-r.left)/r.width,l=Math.min(1,Math.max(0,t));return n+l*(s-n)},[n,s]);c.useEffect(()=>{if(!V)return;if(m){h(null);return}const{index:e,pointerId:r}=V,t=b=>{if(b.pointerId===r){if(m||b.buttons===0){h(null);return}f(e,M(b.clientX))}},l=b=>{b.pointerId===r&&h(null)};return window.addEventListener("pointermove",t),window.addEventListener("pointerup",l),window.addEventListener("pointercancel",l),()=>{window.removeEventListener("pointermove",t),window.removeEventListener("pointerup",l),window.removeEventListener("pointercancel",l)}},[V,m,f,M]);const re=e=>{if(m)return;const r=M(e.clientX),t=i&&Math.abs(r-o[1])<Math.abs(r-o[0])?1:0;f(t,r),h({index:t,pointerId:e.pointerId}),X(e.currentTarget,e.pointerId)},ne=(e,r)=>{m||(e.stopPropagation(),e.currentTarget.focus(),h({index:r,pointerId:e.pointerId}),X(e.currentTarget,e.pointerId))},B=d*10,te=(e,r)=>{if(m)return;const t=o[r];switch(e.key){case"ArrowRight":case"ArrowUp":e.preventDefault(),f(r,t+d);break;case"ArrowLeft":case"ArrowDown":e.preventDefault(),f(r,t-d);break;case"PageUp":e.preventDefault(),f(r,t+B);break;case"PageDown":e.preventDefault(),f(r,t-B);break;case"Home":e.preventDefault(),f(r,n);break;case"End":e.preventDefault(),f(r,s);break}},w=e=>{const r=s-n;return r<=0?0:(e-n)/r*100},le=e=>V?.index===e?3:e===0?2:1,se=H(q,W.slider.track),oe=H(q,W.slider.thumb),ue=i?w(o[0]):0,ie=i?w(o[1])-w(o[0]):w(o[0]),de=i?[0,1]:[0],me=e=>i?e===0?Z||(p?`${p} minimum`:"Minimum value"):J||(p?`${p} maximum`:"Maximum value"):L||(p?void 0:"Slider"),ce=i?`${y(o[0])} – ${y(o[1])}`:y(o[0]);return a.jsxs("div",{id:E,className:x("flex flex-col gap-2",m&&"opacity-50",Q),children:[(p||$)&&a.jsxs("div",{className:"flex items-center justify-between",children:[p&&a.jsx("span",{id:F,className:"text-sm text-muted",children:p}),$&&a.jsx("span",{className:"text-sm font-mono text-accent",children:ce})]}),a.jsxs("div",{ref:U,className:x("relative w-full rounded-full bg-surface shadow-inner",se,m?"cursor-not-allowed":"cursor-pointer"),onPointerDown:re,children:[a.jsx("div",{className:x("absolute top-0 h-full rounded-full transition-[left,width] duration-100",pe[z]),style:{left:`${ue}%`,width:`${ie}%`}}),de.map(e=>a.jsx("div",{role:"slider",tabIndex:m?-1:0,"aria-valuenow":o[e],"aria-valuemin":n,"aria-valuemax":s,"aria-valuetext":y(o[e]),"aria-orientation":"horizontal","aria-disabled":m||void 0,"aria-label":me(e),"aria-labelledby":!i&&!L&&p?F:void 0,className:x("absolute top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full transition-transform","focus:outline-none focus-visible:ring-2 focus-visible:ring-white/80 focus-visible:ring-offset-2 focus-visible:ring-offset-base",m?"cursor-not-allowed":"cursor-grab active:cursor-grabbing hover:scale-110",oe,fe[z]),style:{left:`${w(o[e])}%`,zIndex:le(e)},onPointerDown:r=>ne(r,e),onKeyDown:r=>te(r,e)},e))]})]})};u.displayName="CyberUI.Slider";u.__docgenInfo={description:`A cyberpunk-styled slider for single-value or two-thumb range selection —
  ...
  render: () => {
    const ControlledDemo = () => {
      ...
      return <div className="flex flex-col gap-3 w-80">
          <Slider label="Audio Uplink Volume" value={volume} onValueChange={next => typeof next === 'number' && setVolume(next)} showValue />
...
}`,...A.parameters?.docs?.source}}};const xe=["Default","Primary","Secondary","Range","Sizes","Disabled","Controlled","AllVariants"];export{A as AllVariants,N as Controlled,S as Default,C as Disabled,k as Primary,j as Range,T as Secondary,R as Sizes,xe as __namedExportsOrder,Ve as default};
