...
- **`Table`** component — a cyberpunk-styled, semantic data table for lists of records, deliberately scoped narrow: a single `Table` with `columns`/`data` props (no separate `TableHeader`/`TableRow`/`TableCell` subcomponents), matching `Card`/`Divider`'s existing border and glow treatment. `columns` entries take `{ key, header, render?, align?, width? }` — `render` overrides the default `row[key]` lookup for computed/composite cells. Neon header row (`text-secondary` on a `bg-surface`/`border-accent` bar), an inset-glow hover state on body rows via the existing `--color-secondary` token, and an optional `variant="striped"` alternating row tint. `size` supports responsive values (new `RESPONSIVE_SIZE_MAPS.table` entry) for cell padding/text size. `Table`, `TableProps<T>` and `TableColumn<T>` are generic over the row type, inferred from `data`, so `render`/`getRowId`/`onRowClick` receive typed rows and rows can be declared as `interface`s or contain non-renderable fields like `Date` (a column without `render` only accepts a `key` whose value is directly renderable); `TableRowData` is the default for untyped usage. Optional `caption` (rendered above the table, padded like the cells) or `ariaLabel` for the accessible name, and a `getRowId` accessor for stable row keys. An empty table keeps its shape: the body is about three rows tall at each `size` (new `RESPONSIVE_SIZE_MAPS.tableEmpty` entry), filled with a faint static diagonal hatch (new `bg-no-signal` class) behind the centered `emptyMessage`, which accepts any React node. `onRowClick` makes rows keyboard-focusable (`Tab`, with an accent focus ring) and activatable via click or `Enter`/`Space` pressed on the row itself — controls rendered inside a cell keep their own keyboard behavior — without changing a row's native ARIA semantics. Sorting, filtering, pagination, and virtualization are explicitly out of scope for this first slice (pair with the existing `Pagination` component for paged data).
...
- **Demo Application** — added a new `Console` tab (`src/demo/ConsoleTab.tsx`) curating the 8 components that had never appeared anywhere in the demo app (`Accordion`, `Avatar`, `DropdownMenu`, `FormField`, `Pagination`, `RadioGroup`, `TabNavigation`, `Tooltip`), framed as an operative-records archive terminal split into an "Operative Dossier" and "Archive Navigation" section. Also added `Slider`/`Combobox`/`DatePicker` to `ElementsTab`'s new "Advanced Controls" section and `Drawer` to `InteractiveTab`'s new "Side Panel Access" section, so all 32 components that existed at that point appear somewhere in the demo app instead of only 28. (`Table` isn't in the demo app yet.)

### Fixed

- **⚠️ Published package unusable with any bundler since `2.4.0` (#33)** — `import { Button } from 'cyberui-2045'` failed in Vite, esbuild and Rollup with `UNRESOLVED_IMPORT`. The ESM build (per-module output since `2.4.0`'s tree-shaking fix) emitted bundled copies of `clsx`, `tailwind-merge` and React's JSX runtime under `dist/node_modules/` and `dist/_virtual/`, and imported one shared file from `dist/constants/`, none of which were in the published `files` list (npm also always strips `node_modules` from a tarball). All `dependencies` and `peerDependencies`, including subpaths like `react/jsx-runtime`, are now kept external, and `dist/constants` is published.
- **Library no longer ships its own copy of React's JSX runtime** — the ESM build used to bundle the JSX runtime from whichever React it was built with (React 19), rather than importing the consumer's `react/jsx-runtime`. React 18 is a supported peer dependency, and React 19's JSX runtime emits elements React 18 doesn't recognise. It now imports `react/jsx-runtime` from the consumer's own React.
- **`require('cyberui-2045')` returned an empty object** — the CommonJS entry was a UMD file named `dist/index.js` inside a `"type": "module"` package, so Node loaded it as ESM, where it fell through to its browser-globals branch and exported nothing (silently, since at least `2.3.0`). It's now emitted as `dist/index.cjs`, and `main`/`exports.require` point there.
- **Added `scripts/check-package.js` (`npm run check:package`), run in CI after the library build** — fails if any relative import in a published file points outside the actual `npm pack` file list, if any bare import isn't a declared dependency or peerDependency, or if `require()`-ing the CommonJS entry yields no exports. Each of the three bugs above would have failed it.
- **`npm run type-check` never checked anything** — it ran a plain `tsc --noEmit` against the root `tsconfig.json`, which lists no files and only references the app/node projects, which that command doesn't follow. It always passed, including in CI, and 13 type errors had built up unnoticed (one in `DropdownMenu`, the rest in the `SegmentedProgress` and `Slider` stories). It now checks `tsconfig.app.json` and `tsconfig.node.json` explicitly, and those errors are fixed.
...
- **`Button` (`variant="primary"`) grew 4px in each direction when `disabled` (#35)** — the enabled state had `border-none` and the disabled state `border-2`, so toggling `disabled` shifted the surrounding layout. Disabled primary now draws its outline as a 2px inset ring instead of a border, so both states keep enabled primary's existing size. Enabled primary's size is unchanged.
- **`Button` (`variant="primary"`) no longer blinks when `disabled` toggles** — the gradient was the button's `background-image`, which can't transition, so it cut out instantly and left the button nearly invisible for a frame before the disabled style faded in. The gradient now stays in place in both states, and a dark cover layer fades in over it over the same 300ms as the rest of the button, like the other variants' neon. The gradient is still the button's own background, so overriding it via `className` works as before.
- **⚠️ Regression since `2.3.0` (published) — `Badge` text was invisible for `variant="primary"` when nested in a default-variant `Card`.** `Badge` never set its own text color, relying entirely on inherited `color`; `Card`'s default/small/accent variants all set `text-primary` as a fallback color for children that don't set their own, which exactly matches `variant="primary"`'s `bg-primary` fill, making the label unreadable. Both changes landed in the *same* `2.3.0` commit — one fixing Badge's font-size bug (below) by removing its hardcoded `text-base`, the other adding Card's `text-primary` fallback "so content is readable on dark backgrounds" — each reasonable in isolation, but together they've left this broken in every published version since: `2.3.0`, `2.3.1`, `2.4.0`, and `2.5.0`. Every `Badge` variant now sets its own contrasting text color explicitly via `text-inverse` — a token that was already declared in `src/index.css` for exactly this purpose ("text on a bright/colored fill") but had never actually been used anywhere, so nobody reached for it. Deliberately not `text-base`, which happens to share `--color-base`'s value here but would silently override the `size` prop's `text-xs`/`text-sm` sizing (the exact regression already fixed once before, see the "Badge font size" entry below) — `--color-base` collides with Tailwind's own `base` font-size step, and can't be renamed away from it since it's a published, "guaranteed stable" theming token (README.md's Customization section).
- **Audited every remaining `text-base` usage in the library for the same collision, after the `Badge` regression above.** Testing confirmed the cascade winner between `text-base` and another color class on the same element isn't reliably predictable from reading the code — in different components today, `text-default`/`text-muted`/`text-error` happen to beat `text-base`, while `text-accent` happens to lose to it, for reasons internal to how Tailwind currently orders its generated output, not any rule a future change is guaranteed to preserve. Rather than leave the rest as "currently correct by accident," every `text-base` usage in the library was converted to whichever safe form matches its actual intent — `text-(length:--text-base)` where it's only ever meant to set size (`Badge`, `Checkbox`, `RadioGroup`, `SectionTitle`, `Timeline`, and the `input`/`card`/`skeleton`/`tooltip`/`formField`/`accordion`/`pagination` entries in `RESPONSIVE_SIZE_MAPS`), or `text-inverse` where it's deliberately used for dark-on-bright contrast (`Notification`, `Image`'s preview badge, `Button`, `Pagination`, `DropdownMenu`'s danger-hover state) — `text-inverse` was already declared in `src/index.css` for exactly this purpose but had never been used anywhere until now. No visible change for the cases that already rendered correctly — verified against the demo app and the full test suite — this removes the underlying ambiguity everywhere instead of leaving it to resurface the next time an unrelated change shifts Tailwind's output order.
- **Considered renaming `--color-base` outright** (it's the actual root cause: this project's own token collides with Tailwind's `base` font-size step) but confirmed it's documented in README.md's Customization section as a published, "guaranteed stable across minor versions" theming token, and is present in the published `2.5.0` package — a consumer could have `:root { --color-base: <their color>; }` in their own app right now. Renaming it would silently break that override with no error, which isn't something to fold into a routine bugfix pass; kept the name and treated the collision as permanent instead (see `text-(length:--text-base)`/`text-inverse` above).
