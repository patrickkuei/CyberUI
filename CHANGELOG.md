...
- **`Button` (`variant="primary"`) grew 4px in each direction when `disabled` (#35)** — the enabled state had `border-none` and the disabled state `border-2`, so toggling `disabled` shifted the surrounding layout. Enabled primary now has a transparent 2px border, the same width as the other three variants already use in both states. **Visible change:** enabled primary buttons are 4px wider and taller than before, and now match `secondary`/`danger`/`ghost` exactly. The gradient fill still reaches the outer edge (`bg-origin-border`).
