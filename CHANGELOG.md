...
- **`Button` (`variant="primary"`) no longer blinks when `disabled` toggles** — the gradient was the button's `background-image`, which can't transition, so it cut out instantly and left the button nearly invisible for a frame before the disabled style faded in. The gradient is now a separate layer that fades out over the same 300ms as the rest of the button, like the other variants' neon.
