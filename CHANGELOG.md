...
- **`Table`** component — a cyberpunk-styled, semantic data table for lists of records, deliberately scoped narrow: a single `Table` with `columns`/`data` props (no separate `TableHeader`/`TableRow`/`TableCell` subcomponents), matching `Card`/`Divider`'s existing border and glow treatment. `columns` entries take `{ key, header, render?, align?, width? }` — `render` overrides the default `row[key]` lookup for computed/composite cells. Neon header row (`text-secondary` on a `bg-surface`/`border-accent` bar), an inset-glow hover state on body rows via the existing `--color-secondary` token, and an optional `variant="striped"` alternating row tint. `size` supports responsive values (new `RESPONSIVE_SIZE_MAPS.table` entry) for cell padding/text size. Optional `caption` (rendered above the table) or `ariaLabel` for the accessible name, and a `getRowId` accessor for stable row keys. `onRowClick` makes rows keyboard-focusable (`Tab`) and activatable via click or `Enter`/`Space`, without changing a row's native ARIA semantics. Sorting, filtering, pagination, and virtualization are explicitly out of scope for this first slice (pair with the existing `Pagination` component for paged data).
...
- **`DropdownMenu`'s element trigger is now typed as an element that accepts the injected handlers, ARIA props and ref** — under React 19's types, `cloneElement` on a bare `ReactElement` rejected them. No runtime or public-type change.
...
- **`useDialogBehavior`'s `openDuration` is now optional** (pre-release, caught in issue #30's code review) — `DatePicker` doesn't stage an open animation and never read `isOpening`/`onOpenSettle`, but the hook started a 30ms open-settle timer for it anyway, purely to flip state nothing consumed. Under real timers that timer fired outside any test's `act()` window, producing a spurious warning in `DatePicker.test.tsx`. Omitting `openDuration` now skips the timer/`isOpening` state machinery entirely and settles synchronously instead; only `DatePicker` omits it.
- **`useDialogBehavior`'s `close()` is now idempotent while already closing** (pre-release, caught in issue #30's code review) — Escape and outside-click stay live for the whole close-animation window (`isOpen` doesn't flip to `false` until the close timer completes), and both called `close()` unconditionally. Each repeated call used to clear-and-reschedule the same close timer for a fresh full `closeDuration`, so mashing Escape or repeatedly clicking outside during that window could defer completion indefinitely instead of being a no-op.
...
- **`useDialogBehavior` no longer calls `open()` twice per open cycle** for self-managed callers (`DropdownMenu`, `DatePicker`, `TabNavigation`'s dropdown) — they call it directly from their trigger handler, and the hook's own `isOpen`-transition watcher (meant for externally-controlled callers like `Modal`/`Drawer`) used to redundantly call it again for the same transition. The watcher now skips its own call when the direct one already ran.
- **`Modal` now wires the hook's own `overlayRef`** instead of a separate local ref that was never passed to the hook — previously the hook's internal `overlayRef` stayed permanently `null` for `Modal`, so its `handleOverlayClick`'s primary check (`e.target === overlayRef.current`) never matched and overlay-click-to-close only worked via its `e.target === e.currentTarget` fallback. No behavior change (the fallback already covered every real click), just dead state removed.
