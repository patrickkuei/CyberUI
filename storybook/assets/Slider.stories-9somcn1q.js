import{j as a}from"./jsx-runtime-D_zvdyIk.js";import{r as m}from"./iframe-DrYUIB3m.js";import{g as B,R as H}from"./responsive-DIyY3Grf.js";import{c as x}from"./cn-CNMN3A1O.js";import{w as W}from"./devWarn-C2PD3-_Q.js";import"./preload-helper-D9Z9MdNV.js";const fe={primary:"bg-gradient-to-r from-secondary to-primary shadow-primary",secondary:"bg-gradient-to-r from-primary to-secondary shadow-secondary",accent:"bg-gradient-to-r from-accent to-primary shadow-lg-accent"},ve={primary:"bg-primary shadow-primary",secondary:"bg-secondary shadow-secondary",accent:"bg-accent shadow-lg-accent"};function K(n,s){if(typeof n.setPointerCapture=="function")try{n.setPointerCapture(s)}catch{}}function i({min:n=0,max:s=100,step:d=1,value:I,defaultValue:X,onValueChange:G,variant:P="accent",size:z="md",disabled:c=!1,label:p,showValue:q=!1,formatValue:w=V=>String(V),ariaLabel:$,ariaLabelMin:Z,ariaLabelMax:J,className:Q="",id:Y}){const V=G,ee=m.useId(),L=Y||ee,E=`${L}-label`;s<=n&&W(`slider-invalid-range-${n}-${s}`,`Slider: max (${s}) must be greater than min (${n}) — values will not update correctly.`),d<=0&&W(`slider-invalid-step-${d}`,`Slider: step (${d}) must be greater than 0.`);const[ae,re]=m.useState(()=>I??X??n),D=I!==void 0,g=D?I:ae,u=Array.isArray(g),o=m.useMemo(()=>u?g:[g,g],[u,g]),F=m.useRef(null),[y,h]=m.useState(null),U=m.useRef(o);U.current=o;const O=m.useCallback(e=>{const r=d>0?d:1,l=Math.round((e-n)/r)*r+n,t=Math.min(s,Math.max(n,l));return Math.round(t*1e10)/1e10},[n,s,d]),f=m.useCallback((e,r)=>{const l=O(r),t=[...U.current];u?e===0?t[0]=Math.min(l,t[1]):t[1]=Math.max(l,t[0]):t[0]=l;const v=u?[t[0],t[1]]:t[0];D||re(v),V?.(v)},[u,D,V,O]),M=m.useCallback(e=>{const r=F.current?.getBoundingClientRect();if(!r||r.width===0)return n;const l=(e-r.left)/r.width,t=Math.min(1,Math.max(0,l));return n+t*(s-n)},[n,s]);m.useEffect(()=>{if(!y)return;if(c){h(null);return}const{index:e,pointerId:r}=y,l=v=>{if(v.pointerId===r){if(c||v.buttons===0){h(null);return}f(e,M(v.clientX))}},t=v=>{v.pointerId===r&&h(null)};return window.addEventListener("pointermove",l),window.addEventListener("pointerup",t),window.addEventListener("pointercancel",t),()=>{window.removeEventListener("pointermove",l),window.removeEventListener("pointerup",t),window.removeEventListener("pointercancel",t)}},[y,c,f,M]);const ne=e=>{if(c)return;const r=M(e.clientX),l=u&&Math.abs(r-o[1])<Math.abs(r-o[0])?1:0;f(l,r),h({index:l,pointerId:e.pointerId}),K(e.currentTarget,e.pointerId)},le=(e,r)=>{c||(e.stopPropagation(),e.currentTarget.focus(),h({index:r,pointerId:e.pointerId}),K(e.currentTarget,e.pointerId))},_=d*10,te=(e,r)=>{if(c)return;const l=o[r];switch(e.key){case"ArrowRight":case"ArrowUp":e.preventDefault(),f(r,l+d);break;case"ArrowLeft":case"ArrowDown":e.preventDefault(),f(r,l-d);break;case"PageUp":e.preventDefault(),f(r,l+_);break;case"PageDown":e.preventDefault(),f(r,l-_);break;case"Home":e.preventDefault(),f(r,n);break;case"End":e.preventDefault(),f(r,s);break}},b=e=>{const r=s-n;return r<=0?0:(e-n)/r*100},se=e=>y?.index===e?3:e===0?2:1,oe=B(z,H.slider.track),ie=B(z,H.slider.thumb),ue=u?b(o[0]):0,de=u?b(o[1])-b(o[0]):b(o[0]),ce=u?[0,1]:[0],me=e=>u?e===0?Z||(p?`${p} minimum`:"Minimum value"):J||(p?`${p} maximum`:"Maximum value"):$||(p?void 0:"Slider"),pe=u?`${w(o[0])} – ${w(o[1])}`:w(o[0]);return a.jsxs("div",{id:L,className:x("flex flex-col gap-2",c&&"opacity-50",Q),children:[(p||q)&&a.jsxs("div",{className:"flex items-center justify-between",children:[p&&a.jsx("span",{id:E,className:"text-sm text-muted",children:p}),q&&a.jsx("span",{className:"text-sm font-mono text-accent",children:pe})]}),a.jsxs("div",{ref:F,className:x("relative w-full rounded-full bg-surface shadow-inner",oe,c?"cursor-not-allowed":"cursor-pointer"),onPointerDown:ne,children:[a.jsx("div",{className:x("absolute top-0 h-full rounded-full transition-[left,width] duration-100",fe[P]),style:{left:`${ue}%`,width:`${de}%`}}),ce.map(e=>a.jsx("div",{role:"slider",tabIndex:c?-1:0,"aria-valuenow":o[e],"aria-valuemin":n,"aria-valuemax":s,"aria-valuetext":w(o[e]),"aria-orientation":"horizontal","aria-disabled":c||void 0,"aria-label":me(e),"aria-labelledby":!u&&!$&&p?E:void 0,className:x("absolute top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full transition-transform","focus:outline-none focus-visible:ring-2 focus-visible:ring-white/80 focus-visible:ring-offset-2 focus-visible:ring-offset-base",c?"cursor-not-allowed":"cursor-grab active:cursor-grabbing hover:scale-110",ie,ve[P]),style:{left:`${b(o[e])}%`,zIndex:se(e)},onPointerDown:r=>le(r,e),onKeyDown:r=>te(r,e)},e))]})]})}i.displayName="CyberUI.Slider";i.__docgenInfo={description:`A cyberpunk-styled slider for single-value or two-thumb range selection —
...
@default 100`,defaultValue:{value:"100",computed:!1}},step:{required:!1,tsType:{name:"number"},description:"Increment between selectable values. Also used as the Arrow key step;\nPage Up/Page Down move by `step * 10`.\n@default 1",defaultValue:{value:"1",computed:!1}},value:{required:!1,tsType:{name:"V"},description:"Controlled value. Pass a single number for a single-thumb slider, or a\n`[min, max]` tuple for a two-thumb range slider. Whichever shape is used\non mount (via `value` or `defaultValue`) decides the mode for the\ncomponent's lifetime — switching between number and tuple afterwards is\nnot supported."},defaultValue:{required:!1,tsType:{name:"V"},description:"Initial value for uncontrolled usage, and what determines single- vs\nrange-mode when `value` is never passed. Ignored once `value` is set.\n@default min"},onValueChange:{required:!1,tsType:{name:"signature",type:"function",raw:"(value: NoInferValue<WidenSliderValue<V>>) => void",signature:{arguments:[{type:{name:"tuple[unknown]",raw:"[V][V extends unknown ? 0 : never]"},name:"value"}],return:{name:"void"}}},description:"Fired with the next value on every drag move and keyboard adjustment.\nReceives a `number` for a single-thumb slider and a `[number, number]`\nfor a range slider, matching the shape of `value` / `defaultValue`."},variant:{required:!1,tsType:{name:"union",raw:"'primary' | 'secondary' | 'accent'",elements:[{name:"literal",value:"'primary'"},{name:"literal",value:"'secondary'"},{name:"literal",value:"'accent'"}]},description:`Visual style, reusing the neon palette from Button/Badge/Tooltip.
...
}`,...A.parameters?.docs?.source}}};const Se=["Default","Primary","Secondary","Range","Sizes","Disabled","Controlled","AllVariants"];export{A as AllVariants,N as Controlled,S as Default,R as Disabled,k as Primary,C as Range,T as Secondary,j as Sizes,Se as __namedExportsOrder,xe as default};
