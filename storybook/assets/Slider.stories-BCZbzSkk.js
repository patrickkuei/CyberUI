import{j as a}from"./jsx-runtime-D_zvdyIk.js";import{r as m}from"./iframe-Bn34fd_r.js";import{g as B,R as H}from"./responsive-B9k3icnG.js";import{c as x}from"./cn-CNMN3A1O.js";import{w as W}from"./devWarn-C2PD3-_Q.js";import"./preload-helper-D9Z9MdNV.js";const fe={primary:"bg-gradient-to-r from-secondary to-primary shadow-primary",secondary:"bg-gradient-to-r from-primary to-secondary shadow-secondary",accent:"bg-gradient-to-r from-accent to-primary shadow-lg-accent"},ve={primary:"bg-primary shadow-primary",secondary:"bg-secondary shadow-secondary",accent:"bg-accent shadow-lg-accent"};function K(n,s){if(typeof n.setPointerCapture=="function")try{n.setPointerCapture(s)}catch{}}function i({min:n=0,max:s=100,step:d=1,value:I,defaultValue:X,onValueChange:G,variant:P="accent",size:z="md",disabled:c=!1,label:p,showValue:q=!1,formatValue:w=V=>String(V),ariaLabel:$,ariaLabelMin:Z,ariaLabelMax:J,className:Q="",id:Y}){const V=G,ee=m.useId(),L=Y||ee,E=`${L}-label`;s<=n&&W(`slider-invalid-range-${n}-${s}`,`Slider: max (${s}) must be greater than min (${n}) — values will not update correctly.`),d<=0&&W(`slider-invalid-step-${d}`,`Slider: step (${d}) must be greater than 0.`);const[ae,re]=m.useState(()=>I??X??n),D=I!==void 0,g=D?I:ae,u=Array.isArray(g),o=m.useMemo(()=>u?g:[g,g],[u,g]),F=m.useRef(null),[y,h]=m.useState(null),U=m.useRef(o);U.current=o;const O=m.useCallback(e=>{const r=d>0?d:1,t=Math.round((e-n)/r)*r+n,l=Math.min(s,Math.max(n,t));return Math.round(l*1e10)/1e10},[n,s,d]),f=m.useCallback((e,r)=>{const t=O(r),l=[...U.current];u?e===0?l[0]=Math.min(t,l[1]):l[1]=Math.max(t,l[0]):l[0]=t;const v=u?[l[0],l[1]]:l[0];D||re(v),V?.(v)},[u,D,V,O]),M=m.useCallback(e=>{const r=F.current?.getBoundingClientRect();if(!r||r.width===0)return n;const t=(e-r.left)/r.width,l=Math.min(1,Math.max(0,t));return n+l*(s-n)},[n,s]);m.useEffect(()=>{if(!y)return;if(c){h(null);return}const{index:e,pointerId:r}=y,t=v=>{if(v.pointerId===r){if(c||v.buttons===0){h(null);return}f(e,M(v.clientX))}},l=v=>{v.pointerId===r&&h(null)};return window.addEventListener("pointermove",t),window.addEventListener("pointerup",l),window.addEventListener("pointercancel",l),()=>{window.removeEventListener("pointermove",t),window.removeEventListener("pointerup",l),window.removeEventListener("pointercancel",l)}},[y,c,f,M]);const ne=e=>{if(c)return;const r=M(e.clientX),t=u&&Math.abs(r-o[1])<Math.abs(r-o[0])?1:0;f(t,r),h({index:t,pointerId:e.pointerId}),K(e.currentTarget,e.pointerId)},te=(e,r)=>{c||(e.stopPropagation(),e.currentTarget.focus(),h({index:r,pointerId:e.pointerId}),K(e.currentTarget,e.pointerId))},_=d*10,le=(e,r)=>{if(c)return;const t=o[r];switch(e.key){case"ArrowRight":case"ArrowUp":e.preventDefault(),f(r,t+d);break;case"ArrowLeft":case"ArrowDown":e.preventDefault(),f(r,t-d);break;case"PageUp":e.preventDefault(),f(r,t+_);break;case"PageDown":e.preventDefault(),f(r,t-_);break;case"Home":e.preventDefault(),f(r,n);break;case"End":e.preventDefault(),f(r,s);break}},b=e=>{const r=s-n;return r<=0?0:(e-n)/r*100},se=e=>y?.index===e?3:e===0?2:1,oe=B(z,H.slider.track),ie=B(z,H.slider.thumb),ue=u?b(o[0]):0,de=u?b(o[1])-b(o[0]):b(o[0]),ce=u?[0,1]:[0],me=e=>u?e===0?Z||(p?`${p} minimum`:"Minimum value"):J||(p?`${p} maximum`:"Maximum value"):$||(p?void 0:"Slider"),pe=u?`${w(o[0])} – ${w(o[1])}`:w(o[0]);return a.jsxs("div",{id:L,className:x("flex flex-col gap-2",c&&"opacity-50",Q),children:[(p||q)&&a.jsxs("div",{className:"flex items-center justify-between",children:[p&&a.jsx("span",{id:E,className:"text-sm text-muted",children:p}),q&&a.jsx("span",{className:"text-sm font-mono text-accent",children:pe})]}),a.jsxs("div",{ref:F,className:x("relative w-full rounded-full bg-surface shadow-inner",oe,c?"cursor-not-allowed":"cursor-pointer"),onPointerDown:ne,children:[a.jsx("div",{className:x("absolute top-0 h-full rounded-full transition-[left,width] duration-100 motion-reduce:transition-none",fe[P]),style:{left:`${ue}%`,width:`${de}%`}}),ce.map(e=>a.jsx("div",{role:"slider",tabIndex:c?-1:0,"aria-valuenow":o[e],"aria-valuemin":n,"aria-valuemax":s,"aria-valuetext":w(o[e]),"aria-orientation":"horizontal","aria-disabled":c||void 0,"aria-label":me(e),"aria-labelledby":!u&&!$&&p?E:void 0,className:x("absolute top-1/2 -translate-x-1/2 -translate-y-1/2 rounded-full transition-transform motion-reduce:transition-none","focus:outline-none focus-visible:ring-2 focus-visible:ring-white/80 focus-visible:ring-offset-2 focus-visible:ring-offset-base",c?"cursor-not-allowed":"cursor-grab active:cursor-grabbing hover:scale-110",ie,ve[P]),style:{left:`${b(o[e])}%`,zIndex:se(e)},onPointerDown:r=>te(r,e),onKeyDown:r=>le(r,e)},e))]})]})}i.displayName="CyberUI.Slider";i.__docgenInfo={description:`A cyberpunk-styled slider for single-value or two-thumb range selection —
